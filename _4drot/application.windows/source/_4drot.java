...
public class _4drot extends PApplet {
...
public void setupBrightnessArray() {
...
public void noiseline(float sx, float sy, float sz, float ex, float ey, float ez, int d) {
    if(d < 2) d = 2;
    if(d > 10) d = 10;
...
public void whiteBlur(int blurSize) {
  //validate parms
  if(blurSize < 1) blurSize = 1;
  if(blurSize > 50) blurSize = 50;
  int blurOff = floor(blurSize / 2.0f);
  ...
  for(int y = 0; y < height; y++) {
    ...
    for(int x = 0; x < width; x++, i++) {
      ...
      sum /= PApplet.parseFloat(count);
      img.pixels[i] = color(PApplet.parseInt(sum));
    }
  }
...
public void draw() {
    ...
    for(int i = 0; i < numVertices * 3; i += 3) {
        ...
        if(z > 0) {
            ...
            int h = PApplet.parseInt(50 * z) % 256;
            fill(h, 255, 255);
            ellipse(x, y, 10, 10);
        }
    }
  ...
  static public void main(String args[]) {
    PApplet.main(new String[] { "--bgcolor=#F0F0F0", "_4drot" });
  }
}
